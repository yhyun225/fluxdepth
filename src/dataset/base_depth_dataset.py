...
from src.util.alignment import depth2disparity, disparity2depth
...
class BaseDepthDataset(Dataset):
    ...
    def _training_preprocess(self, rasters, epsilon=1e-8):
        ...
        elif self.target_type == "sqrt_disparity":
            rasters["depth_raw_linear"] = torch.sqrt(
                depth2disparity(rasters["depth_raw_linear"]).clone()
            )
            if self.has_filled_depth:
